...
$bar = function (float $value, ?float $min, ?float $max) {
    ...
    $class = $state === 'ok' ? 'bg-success' : ($state === 'high' ? 'bg-danger' : 'bg-warning text-dark');
...
<div class="mb-3">
    <div class="d-flex justify-content-between align-items-center">
        <a href="<?= site_url('comidas/diario/' . (clone $fechaSel)->modify('-1 day')->format('Y-m-d')) ?>" class="btn btn-sm py-0 btn-outline-secondary">&lt;</a>
        <h6 class="m-0">
            <?= $fechaSel->format('d/m/Y') ?>
        </h6>
        <a href="<?= site_url('comidas/diario/' . (clone $fechaSel)->modify('+1 day')->format('Y-m-d')) ?>" class="btn btn-sm py-0 btn-outline-secondary">&gt;</a>
    </div>
</div>
...
<div class="card mb-3">
    <div class="card-body">
        ...
        <small class="text-muted d-block mb-3">
            Ingeridas: <strong><?= $fmt($kcal, 0) ?> calorias</strong><a href="<?= site_url('comidas/diario/' . $fechaSel->format('Y-m-d') . '/nutrientes') ?>"
                class="btn btn-sm btn-link pb-2">Ver más</a>
        </small>
    ...
    <div class="card mb-2">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div class="d-flex flex-column gap-1">
                ...
                <span class="small">
                    <span class="bg-warning px-1 text-dark"><?= $totK ?> kcal</span>

                    <span class="mx-1">
                        <span class="border-bottom bg-danger px-1">P
                            <?= $totP ?> g</span>
                    </span>

                    <span class="mx-1">
                        <span class="border-bottom bg-primary px-1">C
                            <?= $totC ?> g</span>
                    </span>

                    <span class="mx-1">
                        <span class="border-bottom bg-success px-1">G
                            <?= $totG ?> g</span>
                    </span>
                </span>
